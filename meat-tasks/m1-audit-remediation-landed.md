...
- [ ] Clone `main` to a scratch directory
- [ ] `cp .env.example .env` and fill in a throwaway devnet `AGENT_KEYPAIR`, a real `ANTHROPIC_API_KEY`, a fresh `WEB_CHANNEL_TOKEN` (≥32 chars), `OWNER_WALLET=<your-wallet>`
- [ ] `cp packages/ui/.env.local.example packages/ui/.env.local` and set `NEXT_PUBLIC_WS_TOKEN` to match
- [ ] `pnpm install && pnpm dev:all`
- [ ] Open http://localhost:3001, connect Phantom/Solflare on devnet
- [ ] Send "What is my SOL balance?" — confirm the agent responds
- [ ] Send "Transfer 0.001 SOL to <some-devnet-wallet>" — confirm the approval modal appears, fee line renders, signing completes, signature lands on devnet
